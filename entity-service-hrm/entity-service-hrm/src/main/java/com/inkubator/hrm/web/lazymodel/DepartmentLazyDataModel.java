    ...
    public List<Department> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        ...
        if (sortField != null) {
            if (sortOrder == SortOrder.ASCENDING) {
                try {
                    departments = departmentService.getByParam(departmentSearchParameter, first, pageSize, Order.asc(sortField));
                ...
                try {
                    departments = departmentService.getByParam(departmentSearchParameter, first, pageSize, Order.desc(sortField));
                    jumlahData = Integer.parseInt(String.valueOf(departmentService.getTotalDepartmentByParam(departmentSearchParameter)));
                } catch (Exception ex) {
                    LOGGER.error("Error", ex);
                }
            ...
            try {
                departments = departmentService.getByParam(departmentSearchParameter, first, pageSize, Order.desc("departmentCode"));
                jumlahData = Integer.parseInt(String.valueOf(departmentService.getTotalDepartmentByParam(departmentSearchParameter)));
            } catch (Exception ex) {
                LOGGER.error("Error", ex);
            }
